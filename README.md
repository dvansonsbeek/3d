...
- [Historical Eclipse Validation (Docs 100–103)](docs/100-deltat-validation.md) — The model's ΔT formula (pure-tidal Farhat 2022 + viscoelastic α(t) GIA from Cox & Chao 2002 + Peltier ICE-5G(VM2); **zero parameters fitted to eclipse data**) tested on two independent tracks: **19/19 solar events visible at documented site** vs Stephenson's 17/19 ([Doc 101](docs/101-pure-tidal-eclipses.md)), and **26.7 min mean residual vs NASA's 20.0 min** on 270 primary-source lunar observations ([Doc 102](docs/102-gia-alpha-lunar-validation.md); four traditions — Babylonian, Greek, Chinese, Arab — agree on the residual to ±200 s). The **full** Munk-MacDonald (~5-6 ms/cy) non-tidal postulate is rejected; a dominant GIA-scale channel is included as α(t), and a smaller **fractional non-tidal (~0.5 ms/cy)** is detected in the residual — with the medieval-era shape decomposing structurally into a framework-native **8H/1851 lattice harmonic** (73 × J-S synodic) + fractional drift + observation noise. [Doc 103](docs/103-135-babylonian-case-study.md) decomposes the framework's one persistent umbra-centerline residual (-135 Babylonian).
