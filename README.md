...
- [Historical Eclipse Validation (Docs 100–103)](docs/100-deltat-validation.md) — The model's ΔT formula (pure-tidal Farhat 2022 + **L1-orbital-coupled α(t) GIA** anchored on Cox & Chao 2002 satellite gravimetry and tied to the Climate Formula's L1 orbital layer; **zero parameters fitted to eclipse data**) tested on two independent tracks: a **26-event eclipse alignment audit** on documented solar eclipses spanning -762 BCE to 2026 CE (20/26 events with framework umbra reaching the observation site within a ±4-hour scan window; 4 pure geographic misses — all Ibn Yunus / Said-Stephenson late-tenth-century Cairo observations; see [Doc 101](docs/101-pure-tidal-eclipses.md) for the earlier 19-event methodology and the website's [Solar Eclipse Validation](https://holisticuniverse.com/model/historical-eclipse-validation) for the current audit), and a 270-event primary-source lunar timing test ([Doc 102](docs/102-gia-alpha-lunar-validation.md); **48.6-min mean \|residual\|** on 267 events under the current Espenak 2006-calibrated trend anchor + 4-flag stack; four traditions — Babylonian, Greek, Chinese, Arab — agree on the residual magnitude after detrending). The **full** Munk-MacDonald (~5-6 ms/cy) non-tidal postulate is rejected; a dominant GIA-scale channel (~0.23 ms/cy, Cox & Chao 2002 satellite anchor) is included as α(t), and a smaller **fractional non-tidal (~0.5 ms/cy, ≈ 2× Cox-Chao)** is detected in the residual — with the medieval-era shape decomposing structurally into the framework-native **4-flag 8H-lattice stack (Bond 8H/1830 = 1466 yr + Hallstatt 8H/1104 = 2430 yr + Jose5 8H/2989 = 897 yr + Jose4 8H/3749 = 715 yr)** + fractional drift + observation noise. [Doc 103](docs/103-135-babylonian-case-study.md) documents the -135 Babylonian ΔT-signal case study — BestGap 15 km within the ±4-hour scan, the tightest per-event geographic match in the entire 26-event audit.
