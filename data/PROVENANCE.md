...
| `vsop87a-truncated.json` | VSOP87 version A planetary theory, Bretagnon & Francou (1988), *A&A* **202**, 309 — CDS catalogue VI/81 (`https://cdsarc.cds.unistra.fr/ftp/VI/81/`), truncated under a recorded budget by `tools/pipeline/import-vsop87.js` (raw-file sha256s + truncation parameters in the artifact's `meta`; the K8 standard-model reference overlay, comparison-only). This file is the SINGLE home — the `@essrt/reference` evaluator (private workspace package, one-way boundary) requires it directly; no embedded copy. | CDS-distributed theory series, freely redistributable for scientific use with attribution (standard VizieR terms; the theory paper is the citation) |
