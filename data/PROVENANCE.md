...
| `vsop87a-truncated.json` | VSOP87 version A planetary theory, Bretagnon & Francou (1988), *A&A* **202**, 309 — CDS catalogue VI/81 (`https://cdsarc.cds.unistra.fr/ftp/VI/81/`), truncated under a recorded budget by `tools/pipeline/import-vsop87.js` (raw-file sha256s + truncation parameters in the artifact's `meta`; the K8 standard-model reference overlay, comparison-only). This file is the SINGLE home — the `@essrt/reference` evaluator (private workspace package, one-way boundary) requires it directly; no embedded copy. |
| `elp-mpp02-truncated.json` | ELP/MPP02 lunar theory (Chapront & Francou 2003), truncated from the in-repo series `data/lunar-series/elp-mpp02/` (rights: see the Lunar series section) by `tools/pipeline/import-elp-mpp02.js` — truncation MEASURED against the full port (residuals in `meta`), with the theory's argument polynomials and frame-reduction polynomials baked as numbers. The K8 overlay's Moon ghost; same single-home/one-way pattern as the VSOP87 row. | CDS-distributed theory series, freely redistributable for scientific use with attribution (standard VizieR terms; the theory paper is the citation) |
...
| file | produced by |
|---|---|
| `01-holistic-year-objects-data.xlsx` | The Step 3 browser export (simulator → SheetJS workbook; `tools/fit/README.md` pipeline) — the Earth-fit workbook read by `fit_perihelion_harmonics.py` / `verify_perihelion_erd.py`. Gitignored at 319 MB; regenerate via the browser export before running Phase-2 pipeline steps. **STALE for the planet columns since K5** (plan 06 R3 item 1 check): the export on disk is dated 2026-08-28, before the planets flipped to engine D (P5, 2026-09-06) and the legacy geometric path was excised (K5, 2026-09-08) — it describes the retired planet path. The planet predict device that was trained on those columns (`PREDICT_COEFFS_*`, `planet-prediction-fit-stats.json`) was retired at plan 06 R8 (`docs/retired-record.md`); the Earth columns remain the Step-4 inputs. Re-export from the current simulator before any Earth refit. |
| `insolation-features.csv` | `scripts/extract_insolation_features.js` |
| `insolation-features-deep.csv` | `scripts/extract_insolation_features_deep.js` — the Stage C-2 deep-source feature set (hybrid ε + deep z/ζ + the physical of-date climatic precession), published BESIDE the law CSV, never over it |
| `insolation-stability-deep-results.json` | `INSOL_FEATURES_CSV=data/insolation-features-deep.csv INSOL_STABILITY_OUT=data/insolation-stability-deep-results.json python3 scripts/milankovitch_insolation_stability.py` — the deep-source stability/null verdict (Stage C-2) |
...
| `paleo-validation-anchors.json` | Hand-assembled for the Phase-19 validation dossier: published paleo-rotation and Earth–Moon measurements (Wells 1963 via Arbab 2001, de Winter 2020, Pannella 1972, Williams 2000, Mitchell–Kirscher 2023, Wu et al. 2024, Patterson 1956, Farhat 2022, Zhou 2024, Nanfen 2023, Meyers & Malinverno 2018, Lantink 2022 — the last two also as precession-constant rows, falsification leg 1) transcribed with full citation from the docs/99 validation tables. Observed values are published scientific measurements — facts, reproduced with citation; model predictions are NOT stored, they are recomputed live by the consumer, `tools/verify/paleo-anchors.js` (gate class). |
