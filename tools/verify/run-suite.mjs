...
const MANIFEST = [
  ...
  { n: 'obliquity-hybrid.js', class: 'generator', ms: 90000, slow: true,
    writes: 'data/obliquity-hybrid-verdict.json (tracked; --write only — runs the one-home Stage-C lab: the derived deps/dt(J2000) beside the IAU target, the two-tier eps-vs-La2004 window tables, the H/8 beat). A plain run only prints the banked verdict' },
  ...
  { n: 'earth-zeta-series.js', class: 'generator', ms: 300000, slow: true,
    writes: 'data/nbody-earth-zeta-series.json (tracked; --write only — needs the same untracked ±10-Myr dump as deep-secular-modes). The C-2 ONE-SOURCE orbit-plane history: Earth\'s ζ series resampled at 500-yr cadence, banked with its quality-gate verdict (hybrid ε vs IAU-2006 / La2004 through the one-home factory; refuse-gates on the C-1 class bounds). A plain run only prints the banked verdict' },
