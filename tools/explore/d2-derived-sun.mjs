// STAGE D2 PHASE C — the DERIVED Sun planetary series, epoch-phased
// (plan §12i item 10; LANDED — feeds the shipped v2 table via
// d2-sun-table-extraction.mjs).
//
// Twin systems with REAL t0 phases (t0 = JD(1900.0); framework planet
// longitudes read from the engine graph, validated 14–47′ vs JPL at
// J2000 — note the graph's angular handedness is MIRRORED vs ecliptic
// λ: λ_abs = λE_true − (λp − λE)_graph):
//   base3 = Sun–Earth–Moon · full = base3 + Mercury/Venus/Mars/Jup/Sat
// dl_sun(t) = geocentric-Sun λ (full − base3), with the SECULAR
// PERIHELION FAMILY PROJECTED OUT (T-modulated annual basis): that
// channel lives in the framework's own ϖ(t)/e(t) laws and must not
// enter the planetary completion — unprojected it leaks ~33″ of
// EoC phase drift (measured).
//
// MEASURED RECORD (2026-08-21, 200 yr @ dt 0.01, first run):
//   JPL Sun residual (extended family removed), 1,600 epochs
//   1900–2100: RMS 8.49″; the derived signal after the same
//   projection: 8.48″; correlation −0.997, slope −0.999; COMBINED
//   RESIDUAL 0.60″ — the derivation explains 99.5% of the variance
//   with zero fitted constants (fitted tables reached ~2.9″).
//   Previews (corrected-sign): centerlines 3.61 → 2.92″ (v3 only),
//   syzygy fleet 5.51 → 4.67 (v3) → 4.02″ joint with the derived
//   Moon extension.
//
// Outputs (beside this script, .local.json, session-durable):
//   d2-sun-jpl-cache.local.json     the 1,600-epoch JPL Sun sample
//   d2-derived-sun-signal.local.json  {jd0, stride, dlP[]} the
//                                     grid-projected derived signal
// Usage: node tools/explore/d2-derived-sun.mjs [years=200] [dt=0.01]
...
const DAY = 86400;
const YEARS = parseFloat(process.argv[2] || '200');
const DT = parseFloat(process.argv[3] || '0.01');
const HERE = fileURLToPath(new URL('.', import.meta.url));

const JD0 = model.time.jdFromYear(1900);
...
console.log('t0 = JD', JD0.toFixed(2), '(1900); planet λ:', Object.entries(lamPlanet).map(([k, v]) => `${k} ${v.toFixed(2)}°`).join('  '));
...
const periE = model.earth.perihelionLongitudeDeg(1900);
const eccE = model.earth.eccentricity(1900);
...
console.log(`integrating base3 + full, ${YEARS} yr @ dt ${DT} ...`);
const t0c = Date.now();
...
writeFileSync(HERE + 'd2-derived-sun-signal.local.json',
  JSON.stringify({ jd0: JD0, stride: B.T[1] - B.T[0], dlP: dlP.map((v) => Math.round(v * 1000) / 1000) }));
console.log(`grid-projected planetary signal RMS ${rms(dlP).toFixed(2)}″ → d2-derived-sun-signal.local.json`);

// ── JPL validation (1,600 epochs, cached) ────────────────────────────────
const CACHE = HERE + 'd2-sun-jpl-cache.local.json';
const J0j = model.time.jdFromYear(1900), J1j = model.time.jdFromYear(2100);
let seed = 19002100;
const rnd = () => { seed = (seed * 1103515245 + 12345) % 2147483648; return seed / 2147483648; };
const jds = Array.from({ length: 1600 }, () => Math.round((J0j + rnd() * (J1j - J0j)) * 1e6) / 1e6).sort((a, b) => a - b);
...
if (!cache || cache.n !== jds.length) {
  ...
  for (let i = 0; i < jds.length; i += 40) {
    const batch = jds.slice(i, i + 40);
    const url = 'https://ssd.jpl.nasa.gov/api/horizons.api?format=text'
      + `&COMMAND='10'&CENTER='500@399'&EPHEM_TYPE=OBSERVER&QUANTITIES='31'&CSV_FORMAT=YES&TLIST='${batch.join(' ')}'`;
    const txt = await (await fetch(url)).text();
    const rowsT = txt.split('$$SOE')[1]?.split('$$EOE')[0]?.trim().split('\n') ?? [];
    if (rowsT.length !== batch.length) throw new Error(`batch ${i}`);
    rowsT.forEach((r, k) => out.set(batch[k], parseFloat(r.split(',').map((x) => x.trim())[3])));
    process.stderr.write(`  sun: ${Math.min(i + 40, jds.length)}/${jds.length}\r`);
  }
  cache = { n: jds.length, rows: jds.map((jd) => [jd, out.get(jd)]) };
  writeFileSync(CACHE, JSON.stringify(cache));
  console.log('JPL fetched + cached');
} else console.log('JPL cache reused');
...
console.log(`residual + derived (correct sign): RMS ${rms(r1).toFixed(2)}″`);
