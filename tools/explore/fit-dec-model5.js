#!/usr/bin/env node
// Fit Model 5 Dec correction: A + B/d + (C*sin(u) + D*cos(u) + E*sin(2u) + F*cos(2u))/d
// Output coefficients for constants.js
...
// Disable existing Dec correction to fit from raw errors
...
for (const target of targets) {
  ...
  // Model 5: A + B/d + (C*sin + D*cos + E*sin2 + F*cos2)/d
  const basisFn = pt => [1, 1/pt.d, Math.sin(pt.u)/pt.d, Math.cos(pt.u)/pt.d,
    Math.sin(2*pt.u)/pt.d, Math.cos(2*pt.u)/pt.d];

  const m = 6;
  const X = [], y = [];
  for (const pt of data) { X.push(basisFn(pt)); y.push(pt.dDec); }
  ...
  console.log(`${target.padEnd(10)} n=${String(n).padStart(3)}  orig=${origRMS.toFixed(3)}  corr=${rms.toFixed(3)}  impr=${impr}%`);
