...
// THE CLOCK (verified, plan 06 I1): the cache instants are UT — the cache was
// fetched with a JD TLIST and no TIME_TYPE, which Horizons defaults to UT, and
// three cached epochs re-queried as UT reproduce the cached longitudes exactly
// (as TT they differ by 0.7–0.8 mas·10³, i.e. the ΔT of the epoch). The model
// takes UT and applies its own ΔT, whose anchor convention is 0 at 2000; the
// bridge adds the J2000 ΔT so the model's clock meets the cache's. Reading the
// instants as TT instead (the R1 mistake, reverted here) injects −ΔT(t) into
// the comparison and manufactures a trend of ~4″ across the window.
