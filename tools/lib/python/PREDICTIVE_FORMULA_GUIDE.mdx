...
- **The six physical periods per planet** (ecliptic perihelion, ICRF perihelion, obliquity cycle, ascending node, axial precession, wobble) plus **all pairwise internal and Earth-cross beat frequencies** — derived from the Earth Fundamental Cycle (H) via `planet_beats.py`
...
| Jupiter | H × 8/39 | +1884.2 "/cy |
| Saturn | H × 8/65 | −3140.0 "/cy (retrograde) |
...
(not shipped)                         # Legacy 429-term system (superseded)
├── train_precession.py
├── greedy_features.py
└── *_coeffs_unified.py
...
- **Tweakpane (`src/script.js`):** `npm run constants:generate` — refreshes `PREDICT_COEFFS_PHYSICAL` in the generated constants module, which `src/script.js` imports.
- **Holistic website:** consumes the trained coefficients through the published `@essrt/physics` package — republish after a refit (see `tools/fit/README.md` "Publishing to the website").
