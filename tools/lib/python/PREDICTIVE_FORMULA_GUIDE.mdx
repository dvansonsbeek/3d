...
A unified system for predicting planetary precession fluctuations for all 7 planets (Mercury through Neptune) based on the Holistic Universe Model. (The theoretical-derivation write-up is archived — docs/retired-record.md; this predictive system is the legacy research predecessor, not the shipped path.)
...
Obliquity oscillates with the model-predicted periods. Venus and Neptune return static values (no obliquity cycle).
