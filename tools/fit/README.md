...
> **⚠ THE ENGINE AND THE CSV AGREE (Phase C) — but do NOT refit yet.**
> `data/02-solar-measurements.csv` was produced by the deep-time-ON,
> pos↔JD-integrated engine. Since Phase C restored that engine (R1–R4, R6),
> a 1990–2010 re-export reproduces the committed CSV bit-exactly (worst JD
> delta 0.0000 s, worst `Cycle` delta 0.00e+0) — do not regenerate it,
> 2 h 24 m for an identical file, and it has no git copy.
>
> Steps 6b/6c/6d still MUST NOT run until Phase D lands the fitter-side fixes
> (R10 greedy write-target, R12 integrated phase, R13 SI axis, R14 event-row
> anchor, R15 `Cycle` label) TOGETHER with the runtime evaluation forms.
> Coefficients and runtime form are a MATCHED PAIR; refitting with the current
> fitters is a ~1162-minute-class error
