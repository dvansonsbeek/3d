...
> **⚠ THE CSV IS AHEAD OF THE CODE — do not regenerate it, do not refit from it.**
> `data/02-solar-measurements.csv` carries a `Cycle` column and 9-dp JDs, which
> means it was produced by the deep-time-ON, pos↔JD-integrated engine. That
> engine was reverted; the CSV survived only because it is gitignored, and it
> cost 2 h 24 m with no git copy.
>
> The shipped coefficients predate the CSV and match the CURRENT runtime, so
> code and JSON agree — only the CSV is ahead. Steps 6b/6c/6d run today would
> therefore fit better measurements into a worse runtime. Coefficients and
> runtime form are a MATCHED PAIR; breaking it is a ~1162-minute-class error.
>
> Restoring the engine is the deep-time alignment plan
> (`holisticuniverse/docs/plans/IP-deeptime-scene-graph-alignment_new.md`).
> Until then: back the CSV up, and leave both it and the fits alone.
