...
// ═══════════════════════════════════════════════════════════════════════════
// F. CLIMATE FORMULA COEFFICIENTS (L1 ridge + L2 + L3 per regime)
// ═══════════════════════════════════════════════════════════════════════════
...
if (fs.existsSync(climatePath)) {
  ...
  const beginIdx = src.indexOf(BEGIN_TAG);
  const endIdx   = src.indexOf(END_TAG);
  if (beginIdx === -1 || endIdx === -1 || endIdx < beginIdx) {
    console.log('  (markers not found in script.js — skipping)');
  } else {
    const formatted =
      BEGIN_TAG + '\n' +
      '// Source of truth: public/input/climate-formula-coefficients.json\n' +
      '// Regenerated by: python3 scripts/export_climate_formula_browser.py\n' +
      '// Synced into script.js by: node tools/fit/export-to-script.js --write\n' +
      'const CLIMATE_FORMULA_COEFFS = ' + JSON.stringify(cfm, null, 2) + ';\n' +
      END_TAG;
    const oldBlock = src.slice(beginIdx, endIdx + END_TAG.length);
    if (oldBlock === formatted) {
      console.log('  CLIMATE_FORMULA_COEFFS: unchanged (' +
        Object.keys(cfm.regimes).length + ' regimes)');
    } else {
      console.log('  CLIMATE_FORMULA_COEFFS: ' +
        Object.keys(cfm.regimes).length + ' regimes × (L1+L2+L3) updated');
      src = src.slice(0, beginIdx) + formatted + src.slice(endIdx + END_TAG.length);
      changes++;
    }
  }
} else {
  console.log('  (no climate-formula-coefficients.json found, skipping)');
}
