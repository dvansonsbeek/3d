...
- [Ascending Node Calculations](14-ascending-node-calculations.md) - Full details on the ascending node calculation algorithm
...
The `orbitContainer.rotation` is updated each frame by `updateOrbitalPlaneRotations()` to reflect the current dynamic ascending node. See [Ascending Node Calculations](14-ascending-node-calculations.md) for details.
