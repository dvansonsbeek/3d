...
to 0.03″). The framework twin integration's PLANETARY channel does NOT produce
the slow "~17-yr" MPP02 band (J−S rows ~0.13″) — at the time called
fitted-class floor; the FQ-7 campaign later named and DERIVED it (the
J2 node family — record below).
...
**FQ-7 — the J2 node family (the "~17-yr band" derived after all).**
The band's period is 16.9 yr, not the 18.6-yr J−S beat: it is the Ω
NODE FAMILY — arguments Mp±Ω, 2F+Ω, 2D+Ω, F−Ω — which is not on the
integer Delaunay lattice (Ω enters D/M/Mp/F only in the fixed
combination F − true argument of latitude) and was therefore invisible
to every lattice-catalog fit above. The physical channel is Earth's
oblateness: a twin 3-body integration with the J2 torque on/off
(epoch-phased 1960 ICs, per-run fits on each run's OWN mean angles —
the raw-differential fit leaks A·δrate·t quadrature, a measured trap)
isolates the family, and the two documented J2-class Meeus terms —
λ +1962e-6 sin(Lp−F), β −2235e-6 sin(Lp), present in the head — are
reproduced at ratio 1.028 / 0.971, certifying the channel's amplitude
scale at ±3% and calibrating the run→instrument frame mapping. The
shipped rows (ADD convention, instrument arguments incl. the full Ω
polynomial; the controls do NOT ship — they are already in the head):
λ −0.544 sin(Mp−Ω) + 0.546 sin(Mp+Ω) + 0.371 sin(2F+Ω)
+ 0.103 sin(2D+Ω); β −0.375 sin(F−Ω). Zero fitted constants — every
amplitude is read from the framework integration, dt-halving-converged
≤ 0.003″. Gates (all four, pre-registered): official all-phase JPL
λ 2.96 → 2.92″, β 0.65 → 0.61″; the dense 2-day
JPL arbiter λ 2.38 → 2.32″, β 0.66 → 0.60″; the 179-syzygy fleet
Δ-instrument 9.44 → 9.35″ (Sun completion not subtracted there — the
delta is the signal); NASA centerlines mean 2.7 → 2.4″ / max
6.7 → 5.6″ (Antarctica 3.5 → 2.2″), audit verdicts 3/13/5/0/5
reproduced, Babylon −135 stable at 188 km. What remained of the 20.3h
floor verdict after R3: the beyond-3-body dust
(~1.3″) and the flat ~4″ analytic-vs-DE441 representational gap — the
dust half fell next (the record below).
Instruments: tools/explore/fq7-*.mjs (band probe, node extraction,
dense-JPL fetcher, J2 twin, R3 ship extraction + preview).

**FQ-7 — the dust round (the "~1.3″ broadband floor" was mostly a
catalog blind spot).** The secular-guarded dust census (cubic detrend
plus T/T² quadrature absorbers on the head arguments — without the
guard, the doctrine-blocked T-modulated parameter class masquerades as
fake 184-yr sideband families; measured, v1 vs v2) ranked the post-R3
capturable content, and the dense-JPL arbitration measured EVERY
trusted leader REAL at ratio 0.9–1.1. The labels then bit-matched
MPP02 main-problem catalog rows (amplitude agreement 0.001–0.002″):
the "dust" is ordinary main-problem series content below the Meeus
Table 47 ~0.4″ truncation cutoff, in argument classes no earlier
extraction catalog enumerated — odd kD ([3,0,0,0] 0.40″), |kM| = 3
([2,−3,0,0] 0.34″), |kF| = 4 ([0,0,0,4] 0.42″). Main-problem ⇒ the
D1 3-body lab derives them: one 120-yr run, both columns, head
fidelity 0.51%/0.07%, dt-halving drift 0.000″, lab/census ratio
0.95–1.04, lab signs matching the MPP02 catalog row for row (the
frame-consistency control). The external references keep their
protocol roles — MPP02 discovery-only, JPL out-of-sample-only; every
shipped amplitude is read from the framework integration. SHIPPED:
43 λ + 33 β pure-Delaunay sine rows (content 0.670″/0.464″) appended
to the extension tables — zero fitted constants; the four census rows
the lab does not fully produce (e.g. [2,1,1,−2] at 0.53×) stay
unshipped even though "fitting" them would improve the gates
(stop-gate: derive or don't ship). Gates: official all-phase
λ 2.92 → <!--v:moonSeriesLonVsJplRms-->2.84<!--/v-->″,
β 0.61 → <!--v:moonSeriesLatVsJplRms-->0.35<!--/v-->″ — the β endpoint
...
reproduced, Babylon −135 <!--v:babylon135BestGapKm-->194<!--/v--> km,
centerlines mean flat at <!--v:centerlinesMeanArcsec-->2.2<!--/v-->″
with Antarctica halved 2.2 → 1.0″ and the recorded tension that the
2001 Atlantic max moves 5.6 → <!--v:centerlinesMaxArcsec-->6.5<!--/v-->″
(the known series-tail-anatomy point; the A2-class correlated-subsample
effect — fleet-wide JPL truth governs, the ≤8″ api gate holds). What
remains now is genuinely floor: the ~0.8″ λ post-census residual
(sub-0.04″ deep dust + beyond-3-body), the doctrine-blocked ~5e-5
parameter class, and the analytic-vs-DE441 representational gap.
Instruments: fq7-dust-census.mjs · fq7-dust-arbitration.mjs ·
fq7-dust-extraction.mjs · fq7-dust-preview.mjs.
...
<!--v:centerlinesMaxArcsec-->6.5<!--/v-->″ unchanged), Babylon −135
<!--v:babylon135BestGapKm-->194<!--/v--> km, verdicts 3/13/5/0/5
...
Sun 2.12″ / Moon λ 3.03″ / β <!--v:moonSeriesLatVsJplRms-->0.35<!--/v-->″ (the 20.3h round-2 Delaunay tail later
closed λ to 2.96″, the FQ-7 J2 node family to 2.92″ with β
...
<!--v:centerlinesMeanArcsec-->2.2<!--/v-->″ mean, <!--v:centerlinesMaxArcsec-->6.5<!--/v-->″ max (current audit run, <!--v:centerlinesEvents-->14<!--/v--> events / <!--v:centerlinesPoints-->42<!--/v--> points).
