...
- `tools/fit/export-to-holistic.js` (ΔT section) — patches `BOND_/HALLSTATT_/JOSE5_/JOSE4_ COS_/SIN_COEFF_S`, `_LATTICE_N` and the `RES_*` resonator constants into the website's `deepTime.ts` (absorbed from the deleted `export-dt-corrections.js`). `src/script.js` imports the same values from the generated constants module; `tools/lib/deep-time.js` reads the JSON directly.
