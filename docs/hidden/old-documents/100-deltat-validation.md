...
> **Snapshot note (2026-06-22, last update 2026-07-19)**: This document records the **original pre-α(t) RMS-residual test** (35 events, 584 BCE – 2024 CE) from 2026-06-18. The live model was subsequently extended with an α(t) viscoelastic GIA correction (doc 102, 2026-06-22), shifting the canonical solar mean-residual from 8,658 → 8,682 s (drift of 0.3% — see [doc 101](101-pure-tidal-eclipses.md) line 13). The RMS-residual conclusion is unchanged at the noise floor. **2026-07-19 refresh**: the 19-event visibility-window methodology used below has been superseded by a 26-event eclipse alignment audit (20/26 events with framework umbra reaching the observation site within a ±4h scan window; 4/26 pure geographic misses — all Ibn Yunus / Said-Stephenson late-tenth-century Cairo observations). The complementary lunar timing test now reports a **mean |residual| of 48.6 min** on 267 events under the current Espenak-calibrated trend anchor + 4-flag stack (see [doc 102](102-gia-alpha-lunar-validation.md) headline). The "pure-tidal without non-tidal speedup" framing used below predates the α(t) work; the current canonical framing — Munk-MacDonald-scale (~5-6 ms/cy) non-tidal speedup rejected, GIA-scale (~0.23 ms/cy, Cox & Chao 2002 satellite anchor) contribution included via α(t), plus a smaller fractional non-tidal secular rate (~0.5 ms/cy, ≈ 2× Cox-Chao) detected in the residual — lives in [doc 102](102-gia-alpha-lunar-validation.md) and the website's [Lunar Eclipse Validation](https://holisticuniverse.com/model/lunar-eclipse-validation) page.
...
|---|---|---|
| Our model vs Stephenson | 16 / 9 / 7 wins (~2° RMS each, statistically indistinguishable) | *(historical, superseded by 2026-07-19 eclipse alignment audit)* 19/19 vs 17/19 penumbra; 8,682 s vs 8,789 s mean residual (model wins both) — the current 26-event eclipse alignment audit reports 20/26 within ±4h scan, 4 pure geographic misses |
