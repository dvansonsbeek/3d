...
## Status
Historical research record (plan 06 T1/T5). This document asked *why* the integer-label comb (periods as integer fractions of an eight-unit base) would be real spectral structure. The pre-registered tests answered: T1 measured the comb against the engine's own beat lines and retired the labels (the shipped climate formula's orbital layer is the physical line set, [doc 92 §2](92-climate-formula.md)); T5 found the ΔT stack's lattice labels carry zero information. The mechanism question survives in physical form — why the engine's secular beats pace the record — and is [doc 109](109-model-nbody-engine-and-lattice-test.md)'s and doc 99's. The text below is kept as written, in the campaign's vocabulary, as the record of the question.
