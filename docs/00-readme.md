...
| # | Document | Description |
|---|----------|-------------|
| 100 | [ΔT model validation via historical solar eclipses](100-deltat-validation.md) | First quantitative RMS-residual comparison of the framework's ΔT against Stephenson-Morrison 2004 and Espenak-Meeus 2006 across 35 historical eclipses (584 BCE – 2024 CE). All three ΔT formulas statistically indistinguishable at ~2° RMS; the framework wins 16/9/7 on per-event nearest-fit. Establishes the noise-floor baseline that the higher-resolution doc 101 + doc 102 tests then improve on |
| 101 | [Pure-tidal Moon physics validates against historical eclipse record](101-pure-tidal-eclipses.md) | *(pre-2026-07-19 historical snapshot; methodology superseded by the 26-event eclipse alignment audit)* Original 19-event visibility-window methodology on cleanly-attributed solar eclipses (-762 to 1654 CE), preserved as historical framing. Structural conclusions (α(t) GIA physics, Munk-MacDonald rejection) remain valid. Current headline: **20/26 events with framework umbra reaching the observation site within a ±4h scan window; 4/26 pure geographic misses** — all Ibn Yunus / Said-Stephenson late-tenth-century Cairo observations |
| 102 | [Pure-tidal + GIA L1-orbital-coupled α(t) validates against historical lunar record](102-gia-alpha-lunar-validation.md) | Higher-resolution lunar-timing test on 270 primary-source observations (Stephenson, Morrison & Hohenkerk 2016; Babylonian, Greek, Chinese, Arab). Current mean \|residual\| **48.6 min** on 267 events under the Espenak 2006-calibrated trend anchor + 4-flag stack. Derives the **L1-orbital-coupled α(t) GIA correction** from independent satellite gravimetry (Cox & Chao 2002) plus the L1 orbital layer of the canonical Climate Formula, with zero parameters fitted to eclipse data in the live model. Full Munk-MacDonald (~5-6 ms/cy) non-tidal-speedup rejected; the α(t) captures the **dominant GIA-scale portion (~0.23 ms/cy, Cox-Chao satellite anchor)**; a **fractional non-tidal secular rate of ~0.5 ms/century** (≈ 2× Cox-Chao; ~10% of full Munk-MacDonald) is present in the residual beyond α(t), consistent with time-variable mantle-core coupling or continental hydrology. Structural diagnostics show the medieval residual decomposes cleanly into: (1) the framework-native **4-flag 8H-lattice stack (Bond 8H/1830 = 1466 yr + Hallstatt 8H/1104 = 2430 yr + Jose5 8H/2989 = 897 yr + Jose4 8H/3749 = 715 yr)**, shipped default-ON — periods are zero-fit structural predictions, amplitudes/phases are calibrated against the ΔT residual as constrained physical priors; (2) the fractional non-tidal secular rate above; (3) observation noise. Under L1-orbital α(t) the eight-hypothesis analysis + follow-up predictive tests (Path A solar-activity coupling, Test 5 planetary-perihelion configuration) collapses correlation-based mechanisms (H3-lunar, Path A, Test 5) to drift-tracking artifacts under per-era analysis — aggregate correlations exist but do not causally explain the medieval bump. The structural mechanisms survive: H6 shows coherent solar-activity detection (three detections all in the same forcing family: Gleissberg 88, Jose 179, de Vries 182 yr); H7 14.2-yr peak partial support (2/3 focused robustness tests pass); the 4-flag lattice stack captures the medieval bump structurally. Framework's independent validation is the 26-event eclipse alignment audit on the website ([Solar Eclipse Validation](https://holisticuniverse.com/model/historical-eclipse-validation)) — 20/26 events with model umbra reaching observation site within ±4h scan, 4 pure geographic misses |
| 103 | [-135 Babylonian solar eclipse case study](103-135-babylonian-case-study.md) | Focused case study of the -135 Apr 15 Babylonian eclipse under the 26-event alignment audit. Current headline: **BestGap 15 km within ±4h scan (tightest per-event match in the audit)**; framework's own greatest-eclipse UT differs from the documented UT by ~48 minutes — the ΔT-signal + off-peak verdict category. Component-level decomposition of the 48-minute ΔT-signal attributes it to Sun ecliptic-longitude drift at antiquity (~640 km umbra offset), framework ΔT vs Stephenson gap (~370 km), framework GMST vs IAU convention gap (~170 km). Meeus Ch. 47 Moon polynomial exonerated (all modern lunar theories converge within 0.001° at year -135). The historical "811 km / 1,159 km / ↶ regional" numbers in the doc reflect earlier framework states and are preserved as historical context |
