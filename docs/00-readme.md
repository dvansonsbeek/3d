...
| 101 | [Pure-tidal Moon physics validates against historical eclipse record](101-pure-tidal-eclipses.md) | Visibility-window methodology on 19 cleanly-attributed solar eclipses (-762 to 1654 CE). Framework's pure-tidal + α(t) GIA ΔT beats Stephenson on penumbra count (19/19 vs 17/19) and on per-event mean residual (8,682 vs 8,789 s). 2026-06-24 Strategy A update box: tighter umbra-centerline criterion gives ★ TOTAL at -584 Thales (73 km), -762 Bur-Sagale (85 km), -708 Confucius (re-attribution, 176 km), -309 Sicily |
| 102 | [Pure-tidal + GIA viscoelastic α(t) validates against historical lunar record](102-gia-alpha-lunar-validation.md) | Higher-resolution lunar-timing test on 270 primary-source observations (Stephenson, Morrison & Hohenkerk 2016; Babylonian, Greek, Chinese, Arab). Mean residual 26.7 min vs NASA Espenak/Meeus 20.0 min under the L1-orbital-coupled α(t) refinement — a 6.7-min gap on top of the ~20-min per-observation noise floor. Derives the α(t) GIA viscoelastic correction from independent satellite gravimetry (Cox & Chao 2002 + Peltier ICE-5G(VM2)) plus the L1 orbital layer of the canonical Climate Formula for the deep-time refinement, with zero parameters fitted to eclipse data in the live model. Full Munk-MacDonald (~5-6 ms/cy) non-tidal-speedup rejected; a **fractional non-tidal secular rate of ~0.5 ms/century** (≈ 2× Cox-Chao satellite value; ~10% of full Munk-MacDonald) is present in the residual beyond the framework's α(t), consistent with time-variable mantle-core coupling or continental hydrology. Structural diagnostics show the medieval residual decomposes cleanly into: (1) a framework-native millennial-scale 8H lattice harmonic at **n=1851 = 73 × Jupiter-Saturn synodic = 1449 yr** — structural prediction from lattice arithmetic, amplitude/phase calibration deferred pending independent paleoclimate source; (2) the fractional non-tidal secular rate above; (3) observation noise + Bond-fit imperfections. Under L1-orbital α(t) the eight-hypothesis analysis + follow-up predictive tests (Path A solar-activity coupling, Test 5 planetary-perihelion configuration) collapses correlation-based mechanisms (H3-lunar, Path A, Test 5) to drift-tracking artifacts under per-era analysis — aggregate correlations exist but do not causally explain the medieval bump. The structural mechanisms survive: H6 shows coherent solar-activity detection (three detections all in the same forcing family: Gleissberg 88, Jose 179, de Vries 182 yr); H7 14.2-yr peak partial support (2/3 focused robustness tests pass); n=1851 lattice harmonic captures the medieval bump structurally |
| 103 | [-135 Babylonian solar eclipse case study](103-135-babylonian-case-study.md) | Focused case study of the framework's one persistent residual at the umbra-centerline criterion: the -135 Apr 15 Babylonian eclipse (1,159 km BestGap). Diagnostic decomposition attributes ~270 km to α(t)-uncloseable ΔT, ~440 km to Meeus Ch. 47 polynomial residual in Moon β (β_Meeus = 0.728° vs β_ELP-2000/82 ≈ 0.66°), ~450 km to other Meeus terms. Resolution: framework's path through Saudi Arabia is correct given its Meeus polynomial; Babylon would have observed a deep partial (~95-99% magnitude); the diary's "Normal Stars visible" is consistent. Cites IMCCE + EclipseWise as external corroboration |
