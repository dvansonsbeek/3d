...
**T7 — the fixed-phase test (pre-registered in plan 06 §4, run on LR04; `scripts/t7_fixed_phase_l1.py`, `data/t7-fixed-phase-l1.json`; the model's own histories from `tools/explore/t7-export-orbital-histories.mjs` → `data/t7-model-orbital-histories.json`).** The 28 lines above ride the engine's own frequencies, but their cos/sin pairs are FITTED per regime — a free amplitude and a free phase per line, 56 numbers — and the GIA α(t) channel (doc 102) inherits those phases. T7 replaced the 56 numbers by the model's own e(t), ε(t) and e·sin ϖ(t) (the one-source movement the scene renders, built at 1 kyr over 0–5.4 Ma) with one coefficient each and ONE climate lag, and scored each LR04 regime out of sample (fit one half of the window, score the other, both orders). Both pre-registered outcomes hold. In-window the fixed-phase model reads R² 0.378 / 0.458 / 0.225 (post-MPT / iNHG–MPT / pre-iNHG) — below the free fit's 0.779 / 0.591 / 0.374, as a constrained subset must, and above the 95th percentile of the phase-randomised null (the same regressors with random Fourier phases: 0.257 / 0.380 / 0.112). Out of sample the fixed-phase model keeps 0.357 / 0.446 / 0.163 — 95 % of its in-window value — while the shipped free-phase L1 has no hold-out skill at half-regime windows at any ridge strength (−4.90 / −7.79 / −8.54 at the shipped λ = 1; the best any λ reaches is +0.02, at a λ that also drops its in-window R² to 0.50). The in-window gap is therefore window-specific description, not transferable structure: the 100-kyr band the free pairs absorb inside post-MPT is not generated by a linear response to e, ε, e·sin ϖ, and the Imbrie & Imbrie ice-volume response to the model's own 65°N insolation (variant c) does not recover it either (0.133 in-window). The fitted climate lag is 5.5–6.5 kyr in every regime — the same 6 kyr the GIA relaxation τ measured independently on the historical ΔT record (plan 06 D7; doc 102). La2004's e, ε, ϖ̄ give the same model 0.380 / 0.458 / 0.220 in-window and 0.362 / 0.442 / 0.135 out of sample — the model's histories and Laskar's are interchangeable here (≤ 0.005). Reading: the per-regime R² quoted in this document are in-window descriptions; the model's own orbital histories carry the transferable part, at roughly half the in-window R² and with real predictive skill. Whether the shipped formula (and α(t) with it) moves to the fixed-phase form is a plan 06 decision; CENOGRID waits for the doc 108 §6 secular solution (the mode-table tail beyond ±10 Myr has no trustworthy phases).
