...
Research synthesis — closes the former TODO item "identify the physical channel for the ~0.5 ms/century fractional non-tidal secular rate." The residual is fully decomposed; its millennial component is identified with the documented core–mantle LOD fluctuation, independently confirmed against archeomagnetic core-flow reconstructions, and found to be representable in-window as the shipped ΔT stack's own lattice difference tones with a physically motivated damping interpretation — quantified in §6 as a driven-damped-resonator formula (T₀ ≈ 3.8–4.0 kyr, low Q, in the published axiMC eigenmode range) with a first guard-passing "channel-4" implementation form. Builds on docs 93 (L1 attribution), 99 (ESSRT), 102 (GIA α), 103 (Babylonian case study). Companion evidence — the tracked result artifacts each section cites below: `data/lod-swing-archeomag-calibration.json`, `data/bond-coreflow-phase-lock-test.json`, `data/deltaT-142yr-window-test.json`, `data/deltaT-swing-difference-tones.json`, `data/core-mantle-resonator-fit.json`, `data/core-mantle-excitation-inversion.json`, `data/core-mantle-channel4-feasibility.json`, `data/deltaT-swing-interaction-network.json`, `data/h26-phase-locked-test.json`, plus the Stage-E / phantom-check instrumentation in `tools/fit/dt-corrections-fit.js`. The campaign's one-off derivation scripts are **not shipped**: they ran against the pre-joint world and cannot reproduce today's numbers (the shipped fit is `dt-corrections-fit.js --joint`, validated by `tools/fit/validate-resonator.js`). These JSONs are the permanent record of what each step measured.

**Update — the swing NOW SHIPS.** After this document was written, the campaign continued (joint fit, impulse-consistent episode) and the Core-mantle swing was integrated as the model's 4th dLOD/dt driver (Resonator), default-ON. §6 closes with the shipped joint-world form ("The shipped form" below — episode constants, joint-fit design, and the anchors that moved with it); §8 records how each of the original shipping constraints was resolved. The campaign's research scripts are not shipped — they ran against the pre-joint world and cannot reproduce today's numbers; each step's findings are preserved in the tracked `data/*.json` artifacts cited below (see the shipped fit `tools/fit/dt-corrections-fit.js --joint`).

---

**Update — the drive tone is RETIRED (plan 06 D7 follow-up).** Once the
GIA channel became the lagged response to the ice history (D7), the swing
was re-measured component by component through the joint fitter
(`DT_RESONATOR_COMPONENTS`, Espenak / full-window RMS): all three
components 12.04 / 21.34 s · no swing 21.79 / 69.58 · the two impulse
kicks only 11.89 / 21.04 · tone only 23.91 / 43.75; dense-target χ²
without the swing 52.8 vs 30.8 with it. The episode is load-bearing, as
much as Bond — but only as the impulse-driven damped oscillation (the two
kicks at −1600 / +1600 through the T₀/Q response). The continuous drive
tone at the bond−hallstatt difference frequency — the "resonance with the
lattice beat" of §6 — buys nothing and ships at amplitude 0 (its dn/phase
kept in the stage-1 JSON as the diagnostic convention). What ships is the
millennial core-mantle EPISODE; §6's difference-tone structure is the
historical record of how the episode was found, not a shipped mechanism.
...
Evidence: `data/lod-swing-archeomag-calibration.json`.
...
Evidence: `data/deltaT-swing-difference-tones.json`.
...
Evidence: `data/core-mantle-resonator-fit.json`.
...
Evidence: `data/deltaT-swing-interaction-network.json`.
...
Evidence: `data/core-mantle-excitation-inversion.json`.
...
Evidence: `data/core-mantle-channel4-feasibility.json`.
