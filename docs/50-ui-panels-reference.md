...
the retired ψ-law cosine at the ICRF-perihelion period.)
...
| Feature | Description |
|---------|-------------|
| **Layer toggles (9)** | **Tidal (L1)** (flat rust dashed, +2.12 ms/cy — pure Moon-recession reference) · **+ GIA (L2)** (teal secular baseline, +1.77 ms/cy at J2000, matches IERS +1.75) · **+ Cycles (L3)** (gold dashed, flags only, −0.13 ms/cy at J2000; gates the ▲/▼ transition markers) · **+ Core-mantle (L4)** (orange bold, −0.08 ms/cy — the shipped observable incl. the Resonator episode) · **Periods** (climate-band shading) · **Bond** (the 1,466-yr Bond harmonic isolated, lavender dashed, default OFF) · **Core** (the Core-mantle swing isolated on the L2 baseline, green, default OFF) · **GISP2** (opt-in proxy overlay, secondary °C axis) · **LR04** (opt-in inverted-δ¹⁸O overlay, green, covers the 200-kyr tab) |
...
The physics and validation methodology live in [doc 102 — GIA α(t) lunar validation](102-gia-alpha-lunar-validation.md) (4-flag stack derivation, Bond IRD comparison, sign-convention analysis), [doc 104 — the millennial rotation swing](104-millennial-rotation-swing.md) (the Core-mantle swing / Resonator episode), and [doc 99 — ESSRT](99-expanding-solar-system-resonance-theory.md) § "dLOD/dt decomposition at J2000" (the layer taxonomy — Tidal / GIA / All cycles / Core-mantle swing / L2 / L3 / L4 — shared by the tweakpane sub-folder, the dashboard export, and this modal).
...
| Feature | Description |
|---------|-------------|
| **Twenty-three categories, six groups** | The dropdown carries group headers in the order the physics builds up, and the header title reads "group · panel": **Earth orbit** — Eccentricity, Perihelion Longitude, Inclination (invariable plane), Ascending Node (invariable plane); **Earth axis** — Obliquity, Axial Precession Period; **Earth cycles** — Precession Periods (all five), Climatic Precession (e·sin ϖ), Summer Insolation (65°N), Milankovitch Overview, Analemma; **Earth clock** — Tropical Year, Sidereal Year, Anomalistic Year, Cardinal Year Lengths, Season Durations, Solar Day Length, ΔT (TT − UT1); **Moon** — Month Lengths, Perigee Precession, Node Regression; **All planets** — Inclination, Eccentricity |
| **Four standard windows** | Every generic category shows the same tab strip — 1000 → 2500 AD · 23,000 BC → 23,000 AD (default) · 250,000 BC → 100,000 AD · 1,000,000 BC → 1,000,000 AD — sampled 200 / 240 / 400 / 800 points so the 41-kyr obliquity cycle keeps ≥ 10 points per cycle at ±1 Myr; ΔT fills the same four slots with its own windows — 1650 → 2050 AD and the Espenak canon's 2000 BC → 3000 AD in seconds, then 250,000 BC → 100,000 AD and ±1 Myr in years (a per-window unit: axis, hover, J2000 table, caption and export all read it; the same curve, re-expressed); the chosen window is one setting for the whole panel, kept when switching categories |
| **Main chart** | Model curve (amber, named "This model" on every generic panel — no qualifier; the convention or route it follows is the Frame's and the Reading's business; a second model-derived line on the same panel is "This model — what differs", e.g. "This model — long-term mean" on Solar Day Length; the two cardinal panels' lines carry quantity names since every line there is the model's own) + every reference formula, each drawn solid inside its declared validity window (`validYears`: the era polynomials ±10 kyr, Vondrák ±200 kyr, Berger ±1 Myr, the La2004 table −250 → +100 kyr, La2010 −500 kyr → 0, Espenak −1999 → 3000, the Stephenson 2016 ΔT spline −720 → 2016) and dotted beyond it; the current-year marker, and the observed / anchor values as points at J2000 |
| **Residual chart** | `reference − model` inside validity, in arcseconds, seconds, milliseconds, degrees or the dimensionless eccentricity depending on the category; collapsible with a "Residual" toggle (one setting for the panel, collapsed by default, kept for the session) — the one place the difference is a curve, since the reference curves lie on top of the model's at the main chart's scale |
| **Hover readout** | A cursor on both charts, moving together, with the year and every shown curve's value evaluated live by the category's own formulas |
...
| **Earth clock · Season Durations** | The four northern seasons of the true Sun in days (spring VE→SS, summer SS→AE, autumn AE→WS, winter WS→VE) against the mean quarter year, from the same one-source cardinal structure (a quarter of the tropical year plus the difference of the two ends' timing offsets); with perihelion on the December solstice (1246 AD) spring = summer and autumn = winter exactly; the Meeus year-2000 lengths (92.76 / 93.65 / 89.84 / 88.99 d) as the J2000 anchors |
...
| **Moon · Month Lengths** | The chain's five months of date as strips on one time axis, each with its own scale (the months differ by two days while their drifts are fractions of a second): the sidereal month — Kepler's third law on the recession distance of date, Driver 1½ directly, lengthening at the tidal rate the LLR-anchored recession sets — and the synodic month (its beat with the sidereal year, so both drivers) on by default; the anomalistic and draconic months (its beats with the perigee and node cycles) and the tropical month (the axial precession folded in) selectable. Each strip against the Meeus Ch. 47 rate of date — tropical 360°/L̇′, sidereal 360°/(L̇′ − ṗ_A) with the IAU 2006 general precession, synodic 360°/Ḋ, anomalistic 360°/Ṁ′, draconic 360°/Ḟ — solid on the canon's −2000 → 3000 range, dotted beyond; the strip label carries the J2000 model and Meeus values and the rms inside validity; hover reads every shown month at one year; the four standard windows; Export prints the current one. Known and stated: Meeus's L′ carries the planetary part of the secular term an orbit-size law does not, so the sidereal slopes part by a fraction of a second at the canon's edges |
| **Moon · Perigee Precession / Node Regression** | The lunar twin of the Earth precession panels: the period of the perigee's advance (~3,231.5 d, 8.85 yr) and of the node's regression (~6,798.4 d, 18.61 yr) against the equinox of date, in days of 86,400 s (as the planet stats — a "year" is Julian in one place and mean solar in another). The one-source line is the chain's Brouwer–Clemence m² route (period ∝ sidereal year² / sidereal month, modulated by Earth's eccentricity of date through the solar perturbation — it dips at every eccentricity maximum, and near J2000 its slope is the secular decrease of Earth's eccentricity, the physics behind Meeus's T² term), bridged from its star-referenced frame to the equinox of date through the model's own axial precession (1/T_date = 1/T_star ± 1/T_p); measured against Meeus over 1000 → 2500 it agrees within 8e-5 yr with the same slope. The chain's of-date cycle counter (a bookkeeping convention with no orbital physics) stays in the engine, unsurfaced — no device line. Reference: the Meeus Ch. 47 mean-argument rates (36,000° over d(L′ − M′)/dT and d(L′ − F)/dT; a J2000-centred polynomial, offered on the canon's −2000 → 3000 range), the registry's J2000 anchors as the observed points |
