...
| Feature | Description |
|---------|-------------|
| **Sixteen categories, five groups** | The dropdown carries group headers in the order the physics builds up, and the header title reads "group · panel": **Earth orbit** — Eccentricity, Perihelion Longitude, Inclination (invariable plane), Ascending Node (invariable plane); **Earth axis** — Obliquity, Axial Precession Period; **Earth cycles** — Precession Periods (all five), Climatic Precession (e·sin ϖ), Analemma; **Earth clock** — Tropical Year, Sidereal Year, Cardinal Year Lengths, Solar Day Length, ΔT (TT − UT1); **All planets** — Inclination, Eccentricity |
...
| **Earth clock · Cardinal Year Lengths** | The four cardinal-point year lengths (VE→VE, SS→SS, AE→AE, WS→WS) and the mean tropical year of date, in SI days like the Tropical and Sidereal Year panels; the residual chart shows each cardinal year minus the mean in seconds (the ±40-s swings that are the panel's content, rms per line in the Reading) |
...
| **Earth-only elsewhere** | The twelve formula categories describe Earth's orbit + spin axis (+ ΔT); per-planet observational validation lives in the WebGeoCalc Explorer and the two all-planet panels |
