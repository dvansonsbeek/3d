...
| Feature | Description |
|---------|-------------|
| **Seventeen categories, five groups** | The dropdown carries group headers in the order the physics builds up, and the header title reads "group · panel": **Earth orbit** — Eccentricity, Perihelion Longitude, Inclination (invariable plane), Ascending Node (invariable plane); **Earth axis** — Obliquity, Axial Precession Period; **Earth cycles** — Precession Periods (all five), Climatic Precession (e·sin ϖ), Analemma; **Earth clock** — Tropical Year, Sidereal Year, Cardinal Year Lengths, Season Durations, Solar Day Length, ΔT (TT − UT1); **All planets** — Inclination, Eccentricity |
| **Four standard windows** | Every generic category shows the same tab strip — 1000 → 2500 AD · 23,000 BC → 23,000 AD (default) · 250,000 BC → 100,000 AD · 1,000,000 BC → 1,000,000 AD — sampled 200 / 240 / 400 / 800 points so the 41-kyr obliquity cycle keeps ≥ 10 points per cycle at ±1 Myr; ΔT keeps two windows only (1000 → 2500 and the Espenak canon's 2000 BC → 3000 AD); the chosen window is one setting for the whole panel, kept when switching categories |
| **Main chart** | Model curve (amber) + every reference formula, each drawn solid inside its declared validity window (`validYears`: the era polynomials ±10 kyr, Vondrák ±200 kyr, Berger ±1 Myr, the La2004 table −250 → +100 kyr, La2010 −500 kyr → 0, Espenak −1999 → 3000) and dotted beyond it; the current-year marker, and the observed / anchor values as points at J2000 |
...
| **Hover readout** | A cursor on both charts, moving together, with the year and every curve's value evaluated live by the category's own formulas |
| **J2000 comparison table** | Every reference formula's value at J2000 + Δ vs Model, with source links |
| **Caption** | The same four blocks on every panel, custom ones included, each its own paragraph under a rule — **Frame**: what is plotted (unit) · the frame or convention · the window; **References**: each reference with what was read from it, its validity window and source link; **Reading**: the numbers only (rms per reference over the visible window, the J2000 anchor with the observed values; on the custom panels their equivalents); **Note**: the physics in a few sentences. An empty block is omitted; numbers live in the Reading, never in the Frame or the Note; the former Model block and the two-point endpoint-difference block are gone |
| **Export** | One "Export" button prints the current window in the paper style (white, its own y range where one was designed for that window, else auto) |
...
| **Earth-only elsewhere** | The thirteen formula categories describe Earth's orbit + spin axis (+ ΔT); per-planet observational validation lives in the WebGeoCalc Explorer and the two all-planet panels |
