...
| Feature | Description |
|---------|-------------|
| **Twenty-four categories, six groups** | The dropdown carries group headers in the order the physics builds up, and the header title reads "group · panel": **Earth orbit** — Eccentricity, Perihelion Longitude, Inclination (invariable plane), Ascending Node (invariable plane); **Earth axis** — Obliquity, Axial Precession Period; **Earth cycles** — Precession Periods (all five), Climatic Precession (e·sin ϖ), Summer Insolation (65°N), Milankovitch Overview, Analemma; **Earth clock** — Tropical Year, Sidereal Year, Anomalistic Year, Cardinal Year Lengths, Season Durations, Solar Day Length, ΔT (TT − UT1); **Moon** — Mean Arguments, Month Lengths, Perigee Precession, Node Regression; **All planets** — Inclination, Eccentricity |
...
| **Moon · Mean Arguments** | The accumulated angles the eclipse chain consumes: the framework-native L′, D, M′ and F of date minus the pure Meeus Ch. 47 polynomials, in arcseconds, one strip per argument on a symmetric scale set by the samples inside the canon, the zero line dashed, the canon's −2000 → 3000 validity shaded, the polynomial dotted beyond it (its T³/T⁴ tails run to degrees there — the polynomial, not the model, is what runs away); its own four windows (1000 → 2500 · the canon · ±10 kyr · ±23 kyr); each label carries the rms and the max inside the canon, the Reading the J2000 values (zero by construction); hover reads all four at one year; Export prints the current window. The Month Lengths and precession panels verify the arguments' rates; this panel is their integral, where a curvature split reads as a growing arc |
