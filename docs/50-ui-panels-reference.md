...
| **Moon · Perigee Precession / Node Regression** | The lunar twin of the Earth precession panels: the period of the perigee's advance (~8.85 yr) and of the node's regression (~18.61 yr) against the equinox of date, in Julian years. The one-source line is the chain's OF-DATE counter (the T·H = const law: cycles per unit ∝ H², period ∝ sidereal year / H — the registry anchor's convention; a slow monotonic drift riding the spin unit); a second, clickable line is the chain's Brouwer–Clemence m² route (period ∝ sidereal year² / sidereal month, modulated by Earth's eccentricity of date through the solar perturbation — it dips at every eccentricity maximum, a ~100-kyr wobble the of-date counter lacks), bridged from its star-referenced frame to the equinox of date through the model's own axial precession (1/T_date = 1/T_star ± 1/T_p) so the two deep-time tiers are read on ONE convention and their split stays visible (measured 0.03 yr rms over ±1 Myr). Reference: the Meeus Ch. 47 mean-argument rates (36,000° over d(L′ − M′)/dT and d(L′ − F)/dT; a J2000-centred polynomial, offered on the canon's −2000 → 3000 range), the registry's J2000 anchors as the observed points |
