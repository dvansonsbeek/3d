...
| Feature | Description |
|---------|-------------|
| **Twenty-one categories, six groups** | The dropdown carries group headers in the order the physics builds up, and the header title reads "group · panel": **Earth orbit** — Eccentricity, Perihelion Longitude, Inclination (invariable plane), Ascending Node (invariable plane); **Earth axis** — Obliquity, Axial Precession Period; **Earth cycles** — Precession Periods (all five), Climatic Precession (e·sin ϖ), Summer Insolation (65°N), Milankovitch Overview, Analemma; **Earth clock** — Tropical Year, Sidereal Year, Cardinal Year Lengths, Season Durations, Solar Day Length, ΔT (TT − UT1); **Moon** — Perigee Precession, Node Regression; **All planets** — Inclination, Eccentricity |
...
| **Moon · Perigee Precession / Node Regression** | The lunar twin of the Earth precession panels: the period of the perigee's advance (~3,231.5 d, 8.85 yr) and of the node's regression (~6,798.4 d, 18.61 yr) against the equinox of date, in days of 86,400 s (as the planet stats — a "year" is Julian in one place and mean solar in another). The one-source line is the chain's Brouwer–Clemence m² route (period ∝ sidereal year² / sidereal month, modulated by Earth's eccentricity of date through the solar perturbation — it dips at every eccentricity maximum, and near J2000 its slope is the secular decrease of Earth's eccentricity, the physics behind Meeus's T² term), bridged from its star-referenced frame to the equinox of date through the model's own axial precession (1/T_date = 1/T_star ± 1/T_p); measured against Meeus over 1000 → 2500 it agrees within 8e-5 yr with the same slope. A clickable second line is the chain's H² cycle counter (the T·H = const device: cycles per unit ∝ H², a bookkeeping convention riding the spin unit, no orbital physics, flat) so the two deep-time tiers stay visible on ONE convention (0.03 yr rms apart over ±1 Myr). Reference: the Meeus Ch. 47 mean-argument rates (36,000° over d(L′ − M′)/dT and d(L′ − F)/dT; a J2000-centred polynomial, offered on the canon's −2000 → 3000 range), the registry's J2000 anchors as the observed points |
| **Earth-only elsewhere** | The fifteen Earth formula categories describe Earth's orbit + spin axis (+ ΔT), the two Moon panels its satellite's precession cycles; per-planet observational validation lives in the WebGeoCalc Explorer and the two all-planet panels |
