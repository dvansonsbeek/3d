...
- Comprehensive hypothesis-testing section: eight alternative mechanisms for the residual were tested rigorously under the L1-orbital α(t) refinement. Six are rejected (mass-balance in three formulations, mantle-core constant coupling, lunar nodal medieval test, and a lagged mass-balance sign-flip). Three show statistical support in different forms: (a) lunar mass-balance integrated Y→2000 is strongly significant (r = −0.381, p < 10⁻⁴, ~4σ, survives Bonferroni) though the solar replication is underpowered at n=89; (b) three of nine literature periodic forcings are detected — Gleissberg 88 yr, Jose 179 yr, and Neptune de Vries 182 yr — **all three in the same forcing family (solar-activity)**, a coherent signature not scattered noise-level hits, suggesting solar activity → ionospheric-thermospheric coupling as a candidate physical channel for the medieval residual; (c) the 14.2-yr peak shows partial support under L1-orbital α(t) (focused-window noise floor and jackknife pass cleanly; half-split narrowly misses). Structural diagnostics further show the medieval residual decomposes into linear secular drift + one symmetric bump in the 840–1020 CE window (exact peak year reference-polynomial-dependent). See doc 102 §"Eight hypotheses tested for the medieval residual" for the full analysis.
