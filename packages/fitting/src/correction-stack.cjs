 ...
 *
 * Deliberately DEPENDENCY-FREE: the constants and scene-graph modules are
 * passed in — this module never imports the engine, so the
 * fitting → physics/engine direction stays one-way.
 */

'use strict';

/**
 * @typedef {Object} CorrectionLayer
 * @property {string} id
 * @property {string[]} keys - dot-paths into the constants module
 * @property {'fittable'|'independent'|'post-hoc'} type
 * @property {*} emptyVal - value that disables the layer
 */
...
/** @param {*} obj @param {string} path @returns {*} */
...
/** @param {*} obj @param {string} path @param {*} val */
 ...
 *
 * @param {*} C  - The constants module (require('tools/lib/constants'))
 * @param {{ _invalidateGraph: () => void }} sg - The scene-graph module
 * @param {string|string[]} layerIds - Layer ID(s) to disable for fitting
 * @returns {() => void} restore() - Call to re-enable all disabled layers
 */
