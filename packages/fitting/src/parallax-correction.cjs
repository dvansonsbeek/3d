...
// 9-3c: moved from tools/fit (shim remains). tools/lib is reached through
// runtime-assembled requires — the bridge that keeps the type checker out
// of pre-migration code.
const path = require('path');
const ROOT = path.resolve(__dirname, '..', '..', '..');
const TOOLS_LIB = path.join(ROOT, 'tools', 'lib');
const { computePlanetPosition } = require(path.join(TOOLS_LIB, 'scene-graph.js'));
const sg = require(path.join(TOOLS_LIB, 'scene-graph.js'));
const C = require(path.join(TOOLS_LIB, 'constants.js'));
const { j2000ToOfDate } = require(path.join(TOOLS_LIB, 'precession.js'));
const refData = require(path.join(ROOT, 'data', 'reference-data.json'));
...
if (process.argv.includes('--write')) {
  const jsonPath = require('path').resolve(ROOT, 'public', 'input', 'fitted-coefficients.json');
