...
// 9-3h: moved from tools/fit (shim remains); tools/lib via the runtime bridge.
const _path0 = require('path');
const _ROOT0 = _path0.resolve(__dirname, '..', '..', '..');
const TOOLS_LIB = _path0.join(_ROOT0, 'tools', 'lib');
const { computePlanetPosition } = require(_path0.join(TOOLS_LIB, 'scene-graph.js'));
const sg = require(_path0.join(TOOLS_LIB, 'scene-graph.js'));
const C = require(_path0.join(TOOLS_LIB, 'constants.js'));
const { j2000ToOfDate } = require(_path0.join(TOOLS_LIB, 'precession.js'));
const refData = require(_path0.join(_ROOT0, 'data', 'reference-data.json'));
