...
// (unused ofDateToJ2000 import deleted at 9-3g — the of-date conversion
// happens inside the engine paths this script calls)

const DATA_PATH = path.resolve(ROOT, 'data', 'reference-data.json');
...
const jpl = require(path.join(TOOLS_LIB, 'horizons-client.js'));
...
const tierCount = {};
for (const [, entries] of Object.entries(refData.planets)) {
  for (const e of entries) {
    const t = e.tier || '?';
