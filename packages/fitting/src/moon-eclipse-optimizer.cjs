...
const ROOT = _path0.resolve(__dirname, '..', '..', '..');
const TOOLS_LIB = _path0.join(ROOT, 'tools', 'lib');
const { computePlanetPosition, thetaToRaDeg, phiToDecDeg, _invalidateGraph } = require(_path0.join(TOOLS_LIB, 'scene-graph.js'));
...
const { baseline: computeJPLBaseline } = require(path.join(TOOLS_LIB, 'optimizer.js'));
