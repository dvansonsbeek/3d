...
// 9-3c: moved from tools/fit (shim remains); tools/lib via the runtime
// bridge that keeps the type checker out of pre-migration code.
const ROOT = path.resolve(__dirname, '..', '..', '..');
const TOOLS_LIB = path.join(ROOT, 'tools', 'lib');
const { baseline } = require(path.join(TOOLS_LIB, 'optimizer.js'));
...
// The restore closure is deliberately unused: this is a process-exit CLI,
// and the disabled state must persist for the whole fit. (An unused d2r
// const also died here when the file joined the linted scope.)
const { prepareForFitting } = require('./correction-stack.cjs');
...
if (process.argv.includes('--write')) {
  const jsonPath = path.resolve(ROOT, 'public', 'input', 'fitted-coefficients.json');
