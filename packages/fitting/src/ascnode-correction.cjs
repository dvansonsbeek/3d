...
const TOOLS_LIB = _path0.join(_path0.resolve(__dirname, '..', '..', '..'), 'tools', 'lib');
const C = require(_path0.join(TOOLS_LIB, 'constants.js'));
const { _invalidateGraph } = require(_path0.join(TOOLS_LIB, 'scene-graph.js'));
const { baseline } = require(_path0.join(TOOLS_LIB, 'optimizer.js'));
