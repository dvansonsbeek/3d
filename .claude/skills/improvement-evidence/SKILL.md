...
description: This project's worked evidence for the continuous-improvement protocol — real corrections, their root causes, and what actually made each stick. Use alongside `continuous-improvement` (the generic protocol) when a correction has just happened and you are deciding whether a rule should change. Carries the finding that a rule ignored twice needs a check, not better wording.
---

# Improvement evidence — what actually made corrections stick here

The generic protocol is the `continuous-improvement` skill from `l-gevity-skills`
(MIT), which owns the method: triggers, root-cause analysis, update discipline,
verification. Its directive 6 says to keep project facts in a local overlay —
**this is that overlay.** It adds no method; it adds the measured instances that
make the method's rules credible, and one finding the generic version states only
abstractly.

## The finding worth carrying

> **A rule ignored twice does not need better wording — it needs to be a check.**

Edit discipline was written in `CLAUDE.md`, in memory, and in the plan, and was
still bypassed. What fixed it was `~/.claude/hooks/block-batch-edits.py`: a hook
that refuses. The generic protocol calls this *"Automation Before Prose"*; this is
