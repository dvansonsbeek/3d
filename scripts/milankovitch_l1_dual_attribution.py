...
def write_reference_doc(rows, DOC_PATH):
    lines = []
    # Frontmatter fields are OWNED by tools/docs/stamp-frontmatter.mjs — emit a
    # minimal block here so a fresh generation never ships without one; the
    # stamper refreshes the values on its next --write run.
    lines.append("---")
    lines.append("docVersion: 1.0")
    lines.append("modelVersion: v11.0")
    lines.append("coefficients: sha256:0000000000000000")
    ...
    lines.append("")
    lines.append("> **Scope note (ESSRT).** The L1 lattice integer-divisor structure (each row "
                 "labelled by n where T = 8H/n) is scale-invariant — the integer n stays constant "
                 "at any epoch. The T (kyr) column values in the tables below and the per-component "
                 "periods of every beat-candidate (Earth.Axial(104) = 8H/104, Jupiter.Peri_ecl(39) "
                 "= 8H/39, etc.) are J2000-evaluated; under "
                 "[ESSRT](99-expanding-solar-system-resonance-theory.md), H(t) evolves at deep time "
                 "via Drivers 1 (LOD growth) and 2 (Kepler), scaling every literal period "
                 "proportionally. Attribution structure (which n corresponds to which beat) is "
                 "permanent; the numerical periods are the J2000 snapshot.")
    lines.append("")
    ...
    lines.append("**Why L2 and L3 are excluded from this doc:** the Earth-planet beat search "
                 ...
                 "divisor of 8H = <!--v:eightH-->2,682,536<!--/v--> yr at J2000) and arises from "
                 "carbon-cycle internal resonance, "
