...
print("  Law 5 balance in R-variables: Σ(in-phase) R_j × C_j = R_Saturn × C_Saturn")
print(f"  where C_j = √m × a^(3/2) × i_mean_rad / √d\n")

sum_203_C_coeff = sum(C_coeff[p] for p in GROUP_203)
print(f"    Σ C_j (in-phase) = {sum_203_C_coeff:.6e}")
...
print(f"\n    Inner planet C sum = {inner_C/sum_203_C_coeff*100:.2f}% of in-phase sum")
